...
!********************************************************************************************
!   code history
!   11/7/2019 raz - allow kelvin effect calculation for pure organic aerosols
!   10/30/2019 raz - fixed bug in surface tension calculation in subr. calculate_kelvin
...
! grows or shrinks size depending on mass increase or decrease
      ...
      do ibin = 1, nbin_a
        if(jaerosolstate(ibin) .ne. no_aerosol)then
          call conform_aerosol_size(ibin)	! BOX
        endif
      enddo
...
! gas
        ...
        gas(iaro3_g)  = cnn(karo3)*conv1
        gas(iaro4_g)  = cnn(karo4)*conv1
        gas(ialk1_g)  = cnn(kalk1)*conv1
        gas(iole1_g)  = cnn(kole1)*conv1
        gas(iapi1_g)  = cnn(kapi1)*conv1
        gas(iapi2_g)  = cnn(kapi2)*conv1
        gas(iapi3_g)  = cnn(kapi3)*conv1
        gas(iapi4_g)  = cnn(kapi4)*conv1
...
! gas
        ...
        cnn(karo3)	= gas(iaro3_g)*conv2
        cnn(karo4)	= gas(iaro4_g)*conv2
        cnn(kalk1)	= gas(ialk1_g)*conv2
        cnn(kole1)	= gas(iole1_g)*conv2
        cnn(kapi1)	= gas(iapi1_g)*conv2
        cnn(kapi2)	= gas(iapi2_g)*conv2
        cnn(kapi3)	= gas(iapi3_g)*conv2
        cnn(kapi4)	= gas(iapi4_g)*conv2
...
! update: oct 2005
!-----------------------------------------------------------------------
      subroutine conform_aerosol_size(ibin)	! TOUCH
!      include 'v33com9a'
      use module_data_mosaic_main, only:  piover6, third
      use module_data_mosaic_aero
...
!!      enddo
      ...
      call MESA(ibin)
!      if(jaerosolstate(ibin) .eq. all_solid)then	! RAZ 11/7/2019: commented out to allow kelvin effect for organic aerosol
!        return						!
!      endif						!
...
! simpler correlation for solution surface tension:
      sigma_soln(ibin) = sigma_water*1000.0 + 4.9*(1. - aH2O_a(ibin)) 	! [dyn/cm]	! Bug fix: RAZ 10/30/2019
!      sigma_soln(ibin) = sigma_water*1000.0 - 49.*(1. - aH2O_a(ibin))	! [dyn/cm] for aerosols containing organics
...
! gas (local) - [ngas_volatile] = ngas_ioa + ngas_soa
      ...
      iaro2_g	= 7	! soa
      iaro3_g	= 8	! soa (secondary organic aerosol)
      iaro4_g	= 9	! soa
      ialk1_g	= 10	! soa
      iole1_g	= 11	! soa
      iapi1_g	= 12	! soa
...
! these indices are order sensitive
      ...
      jaro3	= 28	! insoluble - part of naercomp
      jaro4	= 29	! insoluble - part of naercomp
      jalk1	= 30	! insoluble - part of naercomp
      jole1	= 31	! insoluble - part of naercomp
      japi1	= 32	! insoluble - part of naercomp
      japi2	= 33	! insoluble - part of naercomp
      japi3	= 34	! insoluble - part of naercomp
      japi4	= 35	! insoluble - part of naercomp
      jlim1	= 36	! insoluble - part of naercomp
      jlim2	= 37	! insoluble - part of naercomp
      jh2o	= 38	! water - part of naercomp
...
! names of aer species
      ...
      aer_name(iaro3_a)= "ARO3"
      aer_name(iaro4_a)= "ARO4"
      aer_name(ialk1_a)= "ALK1"
      aer_name(iole1_a)= "OLE1"
      aer_name(iapi1_a)= "API1"
      aer_name(iapi2_a)= "API2"
      aer_name(iapi3_a)= "API3"
      aer_name(iapi4_a)= "API4"
...
! names of gas species
      ...
      gas_name(iaro3_g)	 = "ARO3 "
      gas_name(iaro4_g)	 = "ARO4 "
      gas_name(ialk1_g)	 = "ALK1 "
      gas_name(iole1_g)	 = "OLE1 "
      gas_name(iapi1_g)	 = "API1 "
      gas_name(iapi2_g)	 = "API2 "
      gas_name(iapi3_g)	 = "API3 "
      gas_name(iapi4_g)	 = "API4 "
...
! densities of compounds in g/cc
      ...
      dens_comp_a(jaro3)    = 1.4
      dens_comp_a(jaro4)    = 1.4
      dens_comp_a(jalk1)    = 1.4
      dens_comp_a(jole1)    = 1.4
      dens_comp_a(japi1)    = 1.4 ! 1.0
      dens_comp_a(japi2)    = 1.4 ! 1.0
      dens_comp_a(japi3)    = 1.4
      dens_comp_a(japi4)    = 1.4
...
! densities of generic aerosol species
      ...
      dens_aer_mac(iaro3_a)= 1.4
      dens_aer_mac(iaro4_a)= 1.4
      dens_aer_mac(ialk1_a)= 1.4
      dens_aer_mac(iole1_a)= 1.4
      dens_aer_mac(iapi1_a)= 1.4 ! 1.0
      dens_aer_mac(iapi2_a)= 1.4 ! 1.0
      dens_aer_mac(iapi3_a)= 1.4
      dens_aer_mac(iapi4_a)= 1.4
      dens_aer_mac(ilim1_a)= 1.4
      dens_aer_mac(ilim2_a)= 1.4

!      do iaer = iaro1_a, ilim2_a  ! naer
...
!!           so4   no3   cl    nh4   msa   aro1  aro2  aro3  aro4  alk1  ole1  
...
!    1d sectional code needs a "fixed" dry density
      ...
      kappa_aer_mac(iaro3_a) = 0.1
      kappa_aer_mac(iaro4_a) = 0.1
      kappa_aer_mac(ialk1_a) = 0.1
      kappa_aer_mac(iole1_a) = 0.1
      kappa_aer_mac(iapi1_a) = 0.1
      kappa_aer_mac(iapi2_a) = 0.1
      kappa_aer_mac(iapi3_a) = 0.1
      kappa_aer_mac(iapi4_a) = 0.1
...
! refractive index
      ...
      ref_index_a(jaro3)   = cmplx(1.45,0.)
      ref_index_a(jaro4)   = cmplx(1.45,0.)
      ref_index_a(jalk1)   = cmplx(1.45,0.)
      ref_index_a(jole1)   = cmplx(1.45,0.)
      ref_index_a(japi1)   = cmplx(1.45,0.)
      ref_index_a(japi2)   = cmplx(1.45,0.)
      ref_index_a(japi3)   = cmplx(1.45,0.)
      ref_index_a(japi4)   = cmplx(1.45,0.)
...
! vapor pressures of soa species
      ...
      Po_soa(iaro3_g) = fn_Po(5.7d-5, 156.0d0, T_K)	! [Pascal]
      Po_soa(iaro4_g) = fn_Po(1.6d-3, 156.0d0, T_K)	! [Pascal]
      ...
      Po_soa(iapi3_g) = fn_Po(4.0d-6, 156.0d0, T_K)	! [Pascal]
      Po_soa(iapi4_g) = fn_Po(1.7d-4, 156.0d0, T_K)	! [Pascal]
      Po_soa(ilim1_g) = fn_Po(2.5d-5, 156.0d0, T_K)	! [Pascal]
      Po_soa(ilim2_g) = fn_Po(1.2d-4, 156.0d0, T_K)	! [Pascal]

      sat_factor = 1.0  ! = 1.0 for original SORGAM parameters
...
! local variables
      ...
      real(r8) :: m_api3, m_lim1, kf, kc_api3, kc_lim1, Vp, sum_oa_mass
...
! specify size-dependent bulk diffusivity Db (cm2/s)
	  ...
	  if(iv .lt. iaro1_g)goto 21			! no bulk diffusion limitation for inorganic species
...
! function
      ...
      kappa_org = 0.1
      dum_o = (aer(ilim1_a,jtotal,ibin)*mw_aer_mac(ilim1_a)/dens_aer_mac(ilim1_a) + &
               aer(iapi1_a,jtotal,ibin)*mw_aer_mac(iapi1_a)/dens_aer_mac(iapi1_a) + &
               aer(iapi2_a,jtotal,ibin)*mw_aer_mac(iapi2_a)/dens_aer_mac(iapi2_a) + &
	       aer(iapi3_a,jtotal,ibin)*mw_aer_mac(iapi3_a)/dens_aer_mac(iapi3_a) + &
	       aer(iapi4_a,jtotal,ibin)*mw_aer_mac(iapi4_a)/dens_aer_mac(iapi4_a) + &
