!   code history
!   11/7/2019  raz - bug fix for zero sat_soa(iv). Use max of sat_soa(iv) and 1.e-20.
...
! compute mole fractions of soa species
      sum_soa = 0.0
      do iv = iaro1_g, ngas_volatile
        sum_soa = sum_soa + aer(iv,jp,ibin)			! nmol/m3
      enddo
      sum_soa = sum_soa + aer(ioc_a,jp,ibin)/200.0		! 200 is assumed MW of primary OC
      sum_soa_only = sum_soa

      sum_soa = sum_soa + water_a(ibin)*1.e12/18.0		! add water moles in nmol/m3
...
! check threshold concentration for SOA formation in the absence of primary OC
      if(aer(ioc_a,jp,ibin) .eq. 0.0)then
        sum_dum = 0.0
        do iv = iaro1_g, ngas_volatile
          sum_dum = sum_dum + (gas(iv)+aer(iv,jp,ibin))/max(sat_soa(iv),1.e-20)	! RAZ 11/7/2019: bug fix for zero sat_soa(iv)
        enddo
