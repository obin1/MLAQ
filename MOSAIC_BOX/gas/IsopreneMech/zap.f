...
c----------------------------------------------------------------------- 
     ...
     &          maxextralines=100)
c
      parameter (maxlines = 3*maxrxn    , maxlinech = 65, 
...
c
     ...
     &		indexx,		! species indices              : char: out
...
c
     ...
     &		praduct)	! praduct species in rxn ir    : char: in
c
c------------------------------------------------------------------------
c
...
c
      call create_ppdd(
     &		nspecies,	! number of species            : int : in
     &		indexx,		! species indices              : char: in
     & 		species,	! species names                : char: in
     &		nfixed,		! number of fixed species      : int : in
     &		fixed,		! fixed species names	       : char: in
     &		nrxn,		! number of reactions          : int : in
     &		nreacts,	! no. of reactants in rxn ir   : int : in
     &		nprods,		! no. of praducts in rxn ir    : int : in
     &		reactnt,	! reactant species in rxn ir   : char: in
     &          signn,		! sign of product in rxn ir    : char: in
     &		yield,		! yield of product in rxn ir   : char: in
     &		praduct,	! product species in rxn ir    : char: in
     ...
     &          pp,		! praduction for isp species   : char: out
...
c
     ...
     &		nrxn,		! number of reactions          : int : out
     &		nreacts,	! no. of reactants in rxn ir   : int : out
     &		nprods,		! no. of praducts in rxn ir    : int : out
     &		reactnt,	! reactant species in rxn ir   : char: out
     &          signn,		! sign of product in rxn ir    : char: out
     &		yield,		! yield of product in rxn ir   : char: out
     &		ryield,		! yield of product in rxn ir   : real: out
...
c
c------------------------------------------------------------------------
c create reactnt, signn, yield, and praduct arrays for each reaction...
c
...
c determine ipart
      ...
      if(ipart.eq.110)goto 110		! part 2: praducts
...
c
c check for signn
      if(isignn_state.eq.0)then
         if(dumwd.eq.'+' .or. dumwd.eq.'-')then
           signn(nprods(ir),ir) = dumwd(1:1)
           isignn_state = 1
           goto 601
         elseif(nprods(ir) .eq. 1)then
           ...
           write(6,*)'* no space between signn and species or'
           write(6,*)'* signn missing for a praduct'
...
c
c check for praduct species
      if(ityp(iw,il).eq.3  .and. 
     &   dumwd.ne.'+' .and.
     &   dumwd.ne.'-' .and.
     &   dumwd(1:1).ne.'#' )then
         praduct(nprods(ir),ir) = dumwd
...
c------------------------------------------------------------------------
c creates praduction and destruction arrays
c
...
c------------------------------------------------------------------------
c
c create various praduction and destruction arrays
c
      do 200 isp  = 1, nspecies
	ndd(isp) = 0	! initialize
	npp(isp) = 0	! initialize
      do 200 irxn = 1, nrxn
      ...
      do iprd = 1, nprods(irxn)
        if(praduct(iprd,irxn).eq.species(isp))then
          npp(isp) = npp(isp) + 1
          pp_signn(isp ,npp(isp)) = signn(iprd, irxn)
          pp_yield(isp,npp(isp)) = yield(iprd, irxn)
          pp_rate(isp ,npp(isp)) = rate(irxn)
        endif
      enddo
          if(pp_signn(isp,1)(1:1).eq.'+')then
             pp_signn(isp,1)(1:1) = ''
          endif
c
200   continue
...
c dd strings
           nlen = nbllen(indexx(isp))
           nddlines(isp) = 1		! no. of lines: 1st line
        if(ndd(isp).eq.0)then
           dd(isp,1)=
     &     '      d('//indexx(isp)(1:nlen)//')= 0.0'
        else
           dd(isp,1)=
     &     '      d('//indexx(isp)(1:nlen)//')= '//dd_rate(isp,1)
        endif
c
        do in = 2, ndd(isp)
...
c pp strings
        ...
        if(npp(isp).eq.0)then
           pp(isp,1)=
     &     '      p('//indexx(isp)(1:nlen)//')= 0.0'
        else
           pp(isp,1)=
     &     '      p('//indexx(isp)(1:nlen)//')= '//
c     &            pp_signn(isp,1)(1:nbllen( pp_signn(isp,1) ))//
...
c
      ...
      do ifix = 1, nfixed
        if(praduct(iprd,irxn).eq.fixed(ifix))then
        iphantom = 0
        endif
      enddo
c
     ...
     +		      xval, nchqq, strqq, ipostn)
        irch = strqq(1)(1:nchqq(1))
        nchar1 = nchqq(1)
        nchar2 = nbllen( praduct(iprd,irxn) )
        dumwd = 'praduct side'
        nchar3 = 12
        write(6,8001)irch, praduct(iprd,irxn), dumwd
      endif
...
300   continue
      ...
      write(6,*)'is missing between the signn and the species name'
      write(6,*)'***stopping execution***'
      write(6,*)'   '
      stop
        else
...
c
c right hand side (praducts)
        do 102 iprd = 1, nprods(ir)
        do isp= 1, nspecies
          if(praduct(iprd,ir).eq.species(isp))then
            do ie = 1, nelements(isp)
            if(comp(isp,ie).eq.atom(ia))then
              if(signn(iprd,ir)(1:1).eq.'+')then
                erhs = erhs + elemfactor(isp,ie)*ryield(iprd,ir)
              elseif(signn(iprd,ir)(1:1).eq.'-')then
                erhs = erhs - elemfactor(isp,ie)*ryield(iprd,ir)
              else
...
c
c    calls:
c	isdigit, issignn, isperiod
c---------------------------------------------------------------------

	subroutine linefields(maxwords, ast, nfield, itype, ival,  
     +		              xval, nchqq, strqq, ipostn )

	parameter (maxlinech = 65, maxwordch = 40)
	logical isdigit, issignn, isperiod
...
c...	signn digit - integer or real
c...	period digit - real
c...	signn period digit - real
	    ach1 = ast(ja:ja)
	    ach2 = ast(ja+1:ja+1)
	    ach3 = ast(ja+2:ja+2)
	    if ( isdigit(ach1) .or. 
     +		(issignn(ach1).and.isdigit(ach2)) ) then
		go to 3400
	    else if ( (isperiod(ach1).and.isdigit(ach2)) .or.
     +		    (issignn(ach1).and.isperiod(ach2).and.
     +		    isdigit(ach3)) ) then
		go to 3500
	    else
		go to 3600
...
8000	return
	...
	logical function issignn( ach )
	character*1 ach
	issignn = (ach .eq. '+') .or. (ach .eq. '-')
	return
	end
