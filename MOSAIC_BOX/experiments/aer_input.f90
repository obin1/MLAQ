...
! 
	...
	real mass_AS1, mass_AS2, mass_BC1, mass_BC2, mass_OC1, mass_OC2
	...
	real rho_AS, rho_BC, rho_OC, mw_AS, mw_BC, mw_OC
	real mass_AS, mass_BC, mass_OC, vol_tot, mass_tot 
...
! set mode parameters
	...
	sig_ait  = 1.2
	DpgN_ait = 0.06   ! um
	num_ait  = 5000.0 ! #/cc(air)

	sig_acc  = 1.3
	DpgN_acc = 0.2 ! um
	num_acc  = 500.0 ! #/cc(air)
...
! set species parameters
	...
	ratio_OC_AS1  = 1.8472	! mass_OC/mass_AS
	ratio_OC_AS2  = 1.0


! set up bin structure parameters
      nbin_a  = 70 	! 110	! user specified total number of bins: 10, 30, 60, 120
      Dpmin = 0.0084646	! user specified min diameter (um) = lower boundary of the smallest bin
      Dpmax = 0.8868970	! user specified max diameter (um) = upper boundary of the largest bin
...
!	if(ibin .le. 70)then 
!! set UF and Aitken modes particles as pure BC
!	  mass_BC1 = vol1*rho_BC					! ug/m^3(air)
!	  mass_OC1 = 0.0							! ug/m^3(air)
!	  mass_AS1 = 0.0
!
!	  mass_BC2 = vol2*rho_BC
!	  mass_OC2 = 0.0
!	  mass_AS2 = 0.0
!
!	elseif(ibin .gt. 70)then
! set composition of accumulation mode particles as pure OC
!	  mass_BC1 = 0.0
!	  mass_OC1 = vol1*rho_OC
!	  mass_AS1 = 0.0

	  mass_BC2 = 0.0							! ug/m^3(air)
...
! total
	  ...
	  bc   = mass_BC		! ug/m^3(air)

        write(10,400)ibin, num, Dp*1.0, sigma, jhyst, water, so4, no3, cl, &
...
! calculate overall gas-side mass transfer coefficient
	  ...
	  write(11,411)ibin, Dp, num, dNdlogDp_tot, kg
...
400   format(i5, e12.5, f12.5, f5.1, i4, f8.2, 23(2x, e12.5))
